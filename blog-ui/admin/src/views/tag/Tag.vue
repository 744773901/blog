<template>
  <el-card class="main-card">
    ...
    <div class="operation-container">
      ...
      <el-button @click="isDelete = true" :disabled="this.tagIdList.length === 0" type="danger" size="small"
                 icon="el-icon-delete">批量删除
      </el-button>
      <div style="margin-left: auto">
        <el-input v-model="keyword" @keyup.enter.native="searchTags" @clear="listTags"
                  prefix-icon="el-icon-search" size="small"
                  placeholder="请输入标签名" style="width:200px" clearable/>
        <el-button style="margin-left:1rem" @click="searchTags" type="primary" icon="el-icon-search" size="small">
          搜索
        </el-button>
      </div>
    </div>
    ...
    <el-table border :data="tagList" @selection-change="selectionChange" v-loading="loading">
      ...
      <el-table-column label="操作" width="160px" align="center">
        <template slot-scope="scope">
          ...
          <el-popconfirm title="确定删除吗？" style="margin-left:1rem" @confirm="removeTag(scope.row.id)">
    ...
    <el-dialog :visible.sync="isDelete" width="30%">
      ...
      <div slot="footer">
        ...
        <el-button type="primary" @click="removeTag(null)">
          确 定
        </el-button>
      </div>
    </el-dialog>
...
<script>
import {deleteTag, getAdminTag, saveOrUpdateTag} from "@/api/tag";
...
export default {
  ...
  methods: {
    selectionChange(tagList) {
      this.tagIdList = []
      tagList.forEach(item => this.tagIdList.push(item.id))
    },
    searchTags() {
      this.current = 1
      this.listTags()
    ...
    addOrEditTag() {
      ...
      saveOrUpdateTag(this.tagForm).then(({data}) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message
