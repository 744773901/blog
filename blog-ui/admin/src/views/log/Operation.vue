<template>
  <el-card class="main-card">
    ...
    <el-table v-loading="loading" :data="logList" @selection-change="selectionChange">
      ...
      <el-table-column prop="ipAddress" label="操作地址" align="center" width="130"/>
      <el-table-column prop="ipSource" label="操作地点" align="center" width="150"/>
...
<script>
import {deleteOperationLog, listOperationLog} from "@/api/log";
...
export default {
  ...
  methods: {
    listLogs() {
      ...
      listOperationLog(params).then(({data}) => {
        if (data.flag) {
    ...
    deleteLog(id) {
      ...
      deleteOperationLog(ids).then(({data}) => {
        if (data.flag) {
          this.$notify.success({title: '成功', message: data.message})
          this.listLogs()
        } else {
    ...
    selectionChange(logList) {
      this.logIdList = []
      logList.forEach(item => this.logIdList.push(item.id))
    },
    searchLogs() {
      this.current = 1
      this.listLogs()
