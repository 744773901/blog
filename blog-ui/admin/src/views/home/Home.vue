  ...
  <div>
    ...
    <el-row :gutter="30">
      <el-col :span="6">
        <el-card>
          <div class="card-icon-container">
            <svg-icon icon-class="views" style="color:#40C9C6"/>
          </div>
          <div class="card-desc">
            <div class="card-title">访问量</div>
            <div class="card-count">
              <count-to :startVal="0" :endVal="viewsCount" :duration="duration"/>
            </div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card>
          <div class="card-icon-container">
            <svg-icon icon-class="user-center" style="color:#34BFA3"/>
          </div>
          <div class="card-desc">
            <div class="card-title">用户量</div>
            <div class="card-count">
              <count-to :startVal="0" :endVal="userCount" :duration="duration"/>
            </div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card>
          <div class="card-icon-container">
            <svg-icon icon-class="article-manage" style="color:#F4516C"/>
          </div>
          <div class="card-desc">
            <div class="card-title">文章量</div>
            <div class="card-count">
              <count-to :startVal="0" :endVal="articleCount" :duration="duration"/>
            </div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card>
          ...
          <div class="card-desc">
            ...
            <div class="card-count">
              <count-to :startVal="0" :endVal="messageCount" :duration="duration"/>
            </div>
          </div>
        </el-card>
      </el-col>
...
import {getAdminBlogInfo, getUserArea} from "@/api/home";
...
export default {
  ...
  created() {
    this.getData()
    this.listUserArea()
    //this.userAreaMap.series[0].data = [{name: '北京', value: 99}, {name: '江西', value: 66}, {name: '广东', value: 999}, {name: '未知', value: 20}]
  },
  watch: {
    type() {
      this.listUserArea();
        ...
        if (data.flag) {
          ...
          if (data.data.categoryDTOList !== null) {
            data.data.categoryDTOList.forEach(item => {
              ...
              this.category.series[0].data.push({value: item.articleCount, name: item.categoryName})
            });
          }
          if (data.data.tagDTOList !== null) {
            data.data.tagDTOList.forEach(item => {
              this.tagDTOList.push({id: item.id, name: item.tagName})
            });
          }
          this.loading = false
        }
