...
<script>
import {listMessages, saveMessage} from "../../api/message";
  ...
  methods: {
    ...
    addToList() {
      if (this.messageContent.trim().length === 0) {
        this.$toast({type: 'warning', message: '留言内容不能为空'})
        return false
      }
      const avatar = this.$store.state.avatar ? this.$store.state.avatar : this.$store.state.blogInfo.websiteConfig.touristAvatar
      const nickname = this.$store.state.nickname ? this.$store.state.nickname : '游客'
      let message = {
        avatar,
        nickname,
