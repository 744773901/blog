<template>
  <div>
    ...
    <v-card class="blog-container">
      ...
      <div @click="getTalks" v-if="talkList && count > talkList.length" class="load-wrapper">
...
<script>
import {listTalks, saveTalkLike} from "../../api/talk";
...
export default {
  ...
  methods: {
    getTalks() {
      ...
      listTalks(params).then(res => {
        if (res.flag === true) {
          if (this.current === 1) {
            this.talkList = res.data.records
          } else {
            this.talkList.push(...res.data.records)
          }
