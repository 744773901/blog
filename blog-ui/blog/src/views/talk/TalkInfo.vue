<template>
  <div>
    ...
    <v-card class="blog-container">
      <div class="talk-wrapper">
        <div class="user-info-wrapper">
          ...
          <div class="user-detail-wrapper">
            ...
            <!--图片列表-->
            <v-row class="talk-images" v-if="previewList">
              <v-col
                  :md="4"
                  :cols="6"
      ...
      <!--评论-->
      <talk-comment :type="commentType" @getCommentCount="getCommentCount"/>
    </v-card>
  </div>
</template>

<script>
import {getTalkById, saveTalkLike} from "../../api/talk";
import TalkComment from "../../components/TalkComment";
export default {
  ...
  methods: {
    getTalk() {
      getTalkById(this.$route.params.talkId).then(res => {
        if (res.flag === true) {
          this.talkInfo = res.data
          this.previewList = [] || res.data.imgList
        }
      })
    },
    ...
    like(talk) {
      ...
      saveTalkLike(talk.id).then(res => {
        if (res.flag === true) {
          ...
          this.$store.commit('LIKE_TALK', talk.id)
        }
      });
    },
    previewImg(img) {
  ...
  computed: {
    ...
    isLike() {
      return (id) => {
        ...
        return talkLikeSet.indexOf(id) !== -1 ? '#eb5055' : '#999'
      }
    }
  }
}
