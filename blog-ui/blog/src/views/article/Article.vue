      ...
      <div class="article-info-container">
        ...
        <div class="article-info">
          <div class="first-line">
            <!--发表时间-->
            <span>
              <svg-icon icon-class="date"/> 发表于 {{article.createTime | date}}
            </span>
            <span class="separator">|</span>
            <!--更新时间-->
            <span>
              <svg-icon icon-class="update-time"/> 更新于
            ...
            <span class="article-category">
              <svg-icon icon-class="category2"/>
              <router-link :to="`/categories/${article.categoryId}`"> {{article.categoryName}}</router-link>
            </span>
          </div>
          <div class="second-line">
            <!--字数统计-->
            <span>
              <svg-icon icon-class="statistic"/> 字数统计: {{ wordNum | num }}
            </span>
            <span class="separator">|</span>
            <!--阅读时长-->
            <span>
              <svg-icon icon-class="time"/> 阅读时长: {{readTime}}
            </span>
          </div>
          <div class="third-line">
            ...
            <!--阅读量-->
            <span>
              <svg-icon icon-class="reading"/> 阅读量: {{article.viewsCount}}
            </span>
            <span class="separator">|</span>
            <!--评论数-->
            <span>
              <svg-icon icon-class="comment"/> 评论数: {{commentCount}}
            </span>
          </div>
        </div>
      </div>
            ...
            <div :class="isFull(article.lastArticle.id)" v-if="article.lastArticle.id">
              <router-link :to="`/articles/${article.lastArticle.id}`">
                ...
                <div class="post-info">
                  ...
                  <div class="post-title">{{ article.lastArticle.articleTitle }}</div>
                </div>
              </router-link>
            </div>
            <!--下一篇-->
                ...
                <router-link :to="`/articles/${item.id}`">
                  ...
                  <div class="recommend-info">
                    <div class="recommend-date">
                      <svg-icon icon-class="date"/> {{ item.createTime | date }}
                    </div>
      ...
      <v-col md="3" cols="12" class="d-md-block d-none">
        <div style="position: sticky;top: 20px;">
          ...
          <v-card class="right-container" style="margin-top:20px">
            <div class="right-title">
              <svg-icon icon-class="update-time"/>
...
import {getArticleById, saveArticleLike} from "../../api/article";
...
export default {
  ...
  methods: {
    getArticle() {
      getArticleById(this.$route.params.articleId).then(res => {
        if (res.flag) {
          ...
          this.article.articleContent = markdownToHTML(res.data.articleContent)
          this.$nextTick(() => {
            // 统计文章字数
    ...
    like() {
      ...
      saveArticleLike(this.article.id).then(res => {
        if (res.flag) {
          ...
          this.$store.commit('LIKE_ARTICLE', this.article.id)
        }
      });
    },
